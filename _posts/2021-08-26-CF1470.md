...
$L$ 的求法很秒。考虑从 $L(i+1)$ 转移到 $L(i)$。首先  $i$ 有 $c$ 种翻法。每个 $L$ 维护一个双端队列，考虑翻 $[i,i+j]$ 时的情况，发现要么被放到队列左边，要么放到右边，取决于 $a_{i+j}$草与 $a_i$ 相比的大小。
...
int ql[19],qr[19],s[19][N],sw[19][N];
struct node {
	int l,r,w;
	node(int _l=0,int _r=0,int _w=0) {l=_l,r=_r,w=_w;}
} dq[19][N];
bool cmp(const node &x,const node &y) {return a[x.l]<a[x.r];}
...
signed main() {
	...
	while(T--) {
		...
		rep(i,1,n) a[i]=read();
		rep(k,1,c) {
			ql[k]=300002,qr[k]=300001; rep(i,1,n) s[k][i]=0;
			...
			per(i,n-1,1) {
				...
				else sort(dq[k]+qr[k]+1,dq[k]+qr[k]+dr+1,cmp), qr[k]+=dr;
			}
			rep(i,1,n) s[k][i]=s[k-1][i]+s[k][i];
